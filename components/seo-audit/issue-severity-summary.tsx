...
/**
 * Open-issue counts by severity with quick links into the filtered issue list.
 * Same visual language as the dashboard's issue summary, owned by this module
 * so the current website stays selected when navigating to `/seo-audit/issues`.
 */
export function IssueSeveritySummary({ issues, websiteId }: { issues: SEOIssue[]; websiteId: string }) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {counts.map(({ severity, count }) => (
          <Link
            key={severity}
            href={`/seo-audit/issues?site=${websiteId}&severity=${severity}`}
